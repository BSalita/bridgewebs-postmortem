...
def OHE_Hands(hands_bin: list) -> defaultdict:
...
def Augment_Metric_By_Suits(metrics: pl.DataFrame, metric: str, dtype=pl.UInt8) -> pl.DataFrame:
...
def calculate_scores() -> tuple[dict, dict, pl.DataFrame]:
...
def display_double_dummy_deals(deals: list[Deal], dd_result_tables: list, deal_index: int = 0, max_display: int = 4) -> None:
...
def calc_double_dummy_deals(deals: list[Deal], batch_size: int = 40, output_progress: bool = False, progress: any = None) -> list:
...
def calculate_ddtricks_par_scores(df: pl.DataFrame, hrs_d: dict, scores_d: dict, output_progress: bool = True, progress: any = None) -> tuple[pl.DataFrame, pl.DataFrame, pl.DataFrame]:
...
def generate_single_dummy_deals(predeal_string: str, produce: int, env: dict = dict(), max_attempts: int = 1000000, seed: int = 42, show_progress: bool = True, strict: bool = True, swapping: int = 0) -> tuple[tuple[Deal, ...], list]:
...
def calculate_single_dummy_probabilities(deal: str, produce: int = 100) -> tuple[dict[str, pl.DataFrame], tuple[int, dict]]:
...
def calculate_sd_probs(df: pl.DataFrame, hrs_d: dict, sd_productions: int = 100, progress: any = None) -> tuple[dict, pl.DataFrame]:
...
def get_cached_sd_data(pbn: str, hrs_d: dict) -> dict:
...
def calculate_sd_expected_values(df: pl.DataFrame, hrs_d: dict, scores_df: pl.DataFrame) -> pl.DataFrame:
...
def convert_contract_to_declarer(df: pl.DataFrame) -> list:
...
def AugmentACBLHandRecords(df: pl.DataFrame, hrs_d: dict) -> pl.DataFrame:
...
class HandAugmenter:
    def __init__(self, df: pl.DataFrame, hrs_d: dict = None, sd_productions: int = 40, progress: any = None, lock_func: callable = None):
        self.df = df
    ...
    def perform_hand_augmentations(self) -> pl.DataFrame:
        return self.lock_func(self, self.perform_hand_augmentations_queue_up)
    
    def perform_hand_augmentations_queue_up(self) -> pl.DataFrame:
...
class ScoreAugmenter:
    ...
    def perform_score_augmentations(self) -> pl.DataFrame:
...
class ResultAugmenter:
    def __init__(self, df: pl.DataFrame, hrs_d: dict = None):
        self.df = df
        self.hrs_d = hrs_d if hrs_d is not None else {}
    ...
    def _create_contract_type_booleans(self) -> None:
    ...
    def perform_result_augmentations(self) -> pl.DataFrame:
